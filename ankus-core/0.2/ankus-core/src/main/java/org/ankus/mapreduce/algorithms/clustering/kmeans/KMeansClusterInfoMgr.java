    ...
    */
	public boolean isEqualClusterInfo(KMeansClusterInfoMgr newCluster, double convergeRate)
	{
		...
		while(numericAttrIndexIter.hasNext())
		{
			...
			/*
            if(!newCluster.mNumericValueList.containsKey(attrIndex)) return false;
            else if(!mNumericValueList.get(attrIndex).equals(newCluster.mNumericValueList.get(attrIndex)))
            */
		}
		...
		{	
            ...
            /*
			if(!newCluster.mNominalValueList.containsKey(attrIndex)) return false;
			else
			{
				HashMap<String, Double> valueMap = mNominalValueList.get(attrIndex);
				HashMap<String, Double> clusterValueMap = newCluster.mNominalValueList.get(attrIndex);
